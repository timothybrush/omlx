...
"""Tiny two-rank Nemotron-H graph used by the pipeline diagnostic."""
...
def main() -> int:
    try:
        ...
        if group.size() != 2:
            raise RuntimeError("pipeline smoke requires exactly two ranks")
        ...
        assignments = (
            PipelineAssignment("rank-0", 0, 2, 4, 2, 0, 0, 4),
            PipelineAssignment("rank-1", 1, 0, 2, 2, 0, 0, 4),
        )
        args = ModelArgs(
